...
\node (C) [process, xscale=.5, yscale=.6] {$\scr{C}$};
\node (leftC) [left of=C, xshift=-0.25cm] {$x$};
\node (EQ) [process, right of=C, xscale=.5, yscale=.6, xshift=3.5cm] {$EQ$};
\node (rightEQ) [right of=EQ, xshift=1.5cm] {$\scr{A}_L(x) = \lambda \Longleftrightarrow x \in L$};

\draw [arrow] (leftC) -- (C);

\draw [arrow] (C) -- node[above=2.5mm] {$x_1\$x_2$} node[below=2.5mm] {$x_1x_2 = x$}
    node[below=5mm] {$ ||x_1| - |x_2|| \le 1$} (EQ);

\draw [arrow] (EQ) -- (rightEQ);
\end{tikzpicture}
